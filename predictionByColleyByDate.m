...
tbl_result.PredictionByColley=nan(size(tbl_result.Home));
...
Colley=zeros(size(tbl_teams.teamName));
tmp_tbl_ranking=table(teamName,ScoreFor,ScoreAgainst,ScoreRatio,Win,Lose,WinRatio,Colley);
...
for predictionTargetDate=matchDates'
    ...
    if predictionTargetDate >= predictionFrom
        ...
        tmp_tbl_ranking.Colley=zeros(size(tbl_teams.teamName));
        ...
        for j=1:size(tmp_tbl_result.Home)
            ...
            y=1/2*((tmp_tbl_result_j.HomeScore>tmp_tbl_result_j.AwayScore) ...
                -(tmp_tbl_result_j.HomeScore<tmp_tbl_result_j.AwayScore));  %勝敗
            ...
            % 
            C=X'*X+2*eye(size(X'*X));
            b=X'*y+1;
            rank(C); %フルランク＝逆行列を持つことの確認
            r=inv(C)*b; %レーティングの計算
            
            tmp_tbl_ranking.Colley=r;
        ...
        tbl_result.PredictionByColley(idx,:)=0;
        ...
        for k=1:size(idx)
            if idx(k,:)~=0
                ...
                if tmp_tbl_ranking.Colley(homeIdx,:)~=tmp_tbl_ranking.Colley(awayIdx,:)
                    tbl_result.PredictionByColley(k,:)=tmp_tbl_ranking.Colley(homeIdx,:)>tmp_tbl_ranking.Colley(awayIdx,:);
                elseif tmp_tbl_ranking.ScoreRatio(homeIdx,:)~=tmp_tbl_ranking.ScoreRatio(awayIdx,:)
                    tbl_result.PredictionByColley(k,:)=tmp_tbl_ranking.ScoreRatio(homeIdx,:)>tmp_tbl_ranking.ScoreRatio(awayIdx,:);
                else
                    tbl_result.PredictionByColley(k,:)=tmp_tbl_ranking.WinRatio(homeIdx,:)>tmp_tbl_ranking.WinRatio(awayIdx,:);
                end

                % 予測結果が正しいか判定
                tbl_result.isPredictionCorrect(k,:)=tbl_result.PredictionByColley(k,:)==tbl_result.ActualWin(k,:); 
            end
        end
